...
function Sidebar1({ isCollapsed, toggleSidebar }) {
    ...
    const Menus = [
        ...
        { title: "Settings", icon: <RxGear /> },
    ];

    return (
        <div className="tw:flex">
            <div className={` ${isCollapsed ? "tw:w-0 tw:md:w-20" : "tw:w-72"} tw:bg-gray-500 tw:dark:bg-gray-800 tw:h-full tw:md:p-4 tw:pt-5 tw:absolute tw:md:relative tw:duration-300 shadow-lg`}>
                ...
                <ul className={`tw:pt-6 ${isCollapsed && "ps-2"}`}>
                    {
                        Menus.map((Menu, index) => (
                            <li key={index}
                                className={`tw:flex tw:rounded-md tw:p-2 tw:cursor-pointer hover:tw:bg-light-white tw:text-gray-300 tw:text-lg tw:items-center tw:gap-x-4 ${Menu.gap ? " tw:mt-9" : " tw:mt-2"} ${index === 0 && "tw:bg-light-white"} `}>
